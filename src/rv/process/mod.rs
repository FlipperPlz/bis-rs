...
impl Preprocessor {
      ...
      starting_token: LexToken
        ...
        loop {
            match current_token {
                ...
                LexToken::Hash => {
                    if quoted {
                        Self::continue_output(&mut text_buffer, &mut current_token, &mut output, reader)?;
                    } else {
                        self.handle_hash(reader, &mut text_buffer, &mut current_token, &mut current_line, &mut output)?;
                    }
                }
    ...
    fn handle_hash<I: Read + Seek, O: Write>(&mut self,
      ...
      mut output: &mut Option<&mut O>
        ...
        *current_token = Preprocessor::get_next(reader, &mut text_buffer)?;
        ...
        return if let None = output {
            ...
            match current_token {
                LexToken::Include => self.consume_include_directive(reader, output.as_deref_mut(), &mut text_buffer, &mut current_token, &mut current_line),
                LexToken::Define => self.consume_define_directive(reader, &mut output, &mut text_buffer, &mut current_token, &mut current_line),
                LexToken::IfDef => self.consume_if_block(reader, &mut output, &mut current_token, &mut current_line),
                LexToken::IfNDef => self.consume_if_not_block(reader, &mut output, &mut current_token, &mut current_line),
                LexToken::Undef => self.consume_undefine_directive(reader, &mut output, &mut current_token, &mut current_line),
    ...
    fn consume_include_directive<I: Read + Seek, O: Write>(&mut self,
      ...
      mut current_line: &mut u32
        ...
        if match current_token {
            LexToken::DQuote => Ok(reader.scan_string(&mut text_buffer, 127, CONST_DOUBLE_QUOTE)?),
            LexToken::LeftAngle => Ok(reader.scan_string(&mut text_buffer, 127, CONST_RIGHT_ANGLE)?),
            _ => Err(PreprocessError::InvalidToken {
